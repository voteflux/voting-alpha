...
def handler_priv_keys(event: dict, context, **props):
    ...
    if event['RequestType'] == 'Create':
        ssm_keys, poa_pks, service_pks = create_node_keys(**props)
        data = save_node_keys(ssm_keys, **props)
        data.update(save_poa_pks(poa_pks, **props))
        data.update(save_service_pks(service_pks, **props))
