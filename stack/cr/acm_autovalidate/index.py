...
def handler(event, context):
    ...
    if "domainname" not in properties:
        return responder(event, context, "FAILED", "Missing parameter(s): {domainname}")
    ...
    if event["RequestType"] == "Delete":
        ...
        try:
            ...
            print(f"(change_resource_record_sets) {route53_response}")
    ...
    if not event["PhysicalResourceId"].startswith("arn:aws:acm:"):
        ...
        try:
            ...
            traceback.print_exc()
            # it's unlikely acm_response exists so let's bail out
            return responder(event, context, status="FAILED", reason=traceback.format_exc()[:700])
    ...
    while True:
        try:
            certificate_details = acm_client.describe_certificate(CertificateArn=event["PhysicalResourceId"])
            print(f"certificate_details: {certificate_details}")
            change_record = [
                {
                    "Action": "UPSERT",
                    "ResourceRecordSet": {
            ...
            exceptions += 1
            print(f'Got an exception during describe_cert loop: {repr(e)}')
            time.sleep(10)
        if exceptions >= 3:
            break
    try:
        print(f"change_record operations: {change_record}")
